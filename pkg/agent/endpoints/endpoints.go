...
import (
	...
	sdsv2 "github.com/spiffe/spire/pkg/agent/endpoints/sds"
	sdsv3 "github.com/spiffe/spire/pkg/agent/endpoints/sdsv3"
...
func (e *Endpoints) registerSecretDiscoveryService(server *grpc.Server) {
	...
	h := sdsv2.NewHandler(sdsv2.HandlerConfig{
		Attestor:          attestor,
		Manager:           e.c.Manager,
	...
	discovery_v2.RegisterSecretDiscoveryServiceServer(server, h)

	hv3 := sdsv3.NewHandler(sdsv3.HandlerConfig{
		Attestor:          attestor,
		Manager:           e.c.Manager,
		Log:               e.c.Log.WithField(telemetry.SubsystemName, telemetry.SDSAPI),
		Metrics:           e.c.Metrics,
	...
	secret_v3.RegisterSecretDiscoveryServiceServer(server, hv3)
}
